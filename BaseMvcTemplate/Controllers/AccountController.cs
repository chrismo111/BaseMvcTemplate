...
namespace BaseMvcTemplate.Controllers {
    ...
    public class AccountController: Controller {
        ...
        public async Task<ActionResult> Login( LoginViewModel model, string returnUrl ) {
            ...
            //Regex pattern taken from: http://msdn.microsoft.com/en-us/library/01escwtf(v=vs.110).aspx
            string emailRgex = @"^(?("")("".+?(?<!\\)""@)|(([0-9a-z]((\.(?!\.))|[-!#\$%&'\*\+/=\?\^`\{\}\|~\w])*)(?<=[0-9a-z])@))" +
                 @"(?(\[)(\[(\d{1,3}\.){3}\d{1,3}\])|(([0-9a-z][-\w]*[0-9a-z]*\.)+[a-z0-9][\-a-z0-9]{0,22}[a-z0-9]))$";

            if( Regex.IsMatch( model.Email, emailRgex, RegexOptions.IgnoreCase, TimeSpan.FromMilliseconds( 250 ) ) ) {
                using( var db = new ApplicationDbContext() ) {
